...
- V0.5 已完成默认测试范围：CDC stub、ChangeEvent 标准化和本地上传队列发布入口。
- V0.6 已完成默认测试范围：CDC offset 模型和 Canal adapter 前置接口。
- V0.7 已完成默认测试范围：MySQL offset store 和 CDC 异常恢复策略。
- V0.8 已完成默认测试范围：ACK、dispatch、error 持久化和失败事件入口。
- V0.9 已完成默认测试范围：事件载荷持久化和待重放事件单步 replay。
- V0.10 已完成默认测试范围：Server 后台自动 replay worker。
- V0.11 已完成默认测试范围：单机 lab 拓扑、配置和准备脚本。
- V0.12 已完成默认测试范围：单机 E2E smoke 脚本。
- V0.13 已完成默认测试范围：真实 Canal client 适配层。
- V0.14 已进入实现：Canal adapter 接入 Edge runtime 和测试 CLI。
...
| V0.15 | Windows Service | SyncAgent 可安装、启动、停止、卸载为 Windows Service |
| V0.16 | Wails MVP | 配置、状态、规则、失败事件、日志页面可用且默认不占前端端口 |
| V0.17 | Diagnostics + installer runbook | 诊断包和安装交付流程完成 |
...
- 客户试用最早在 V0.17 后：Windows Service、Wails MVP、诊断包和安装 runbook 必须具备。
