...
- V0.13 已进入实现：真实 Canal client 适配层。
...
| V0.14 | Windows Service | SyncAgent 可安装、启动、停止、卸载为 Windows Service |
| V0.15 | Wails MVP | 配置、状态、规则、失败事件、日志页面可用且默认不占前端端口 |
| V0.16 | Diagnostics + installer runbook | 诊断包和安装交付流程完成 |
...
- 暂不把真实 Canal 接入 `sync-agent run`，先保留 stub CDC E2E。
...
- 客户试用最早在 V0.15 后：Windows Service、Wails MVP、诊断包和安装 runbook 必须具备。
