...
- V0.10 已进入实现：Server 后台自动 replay worker。
...
| V0.11 | Real CDC adapter | 真实 Canal client 路径确认并可读取 MySQL binlog |
| V0.12 | E2E lab smoke | Docker MySQL/RabbitMQ 下 Edge A -> Server -> Edge B 跑通 |
| V0.13 | Windows Service | SyncAgent 可安装、启动、停止、卸载为 Windows Service |
| V0.14 | Wails MVP | 配置、状态、规则、失败事件、日志页面可用且默认不占前端端口 |
| V0.15 | Diagnostics + installer runbook | 诊断包和安装交付流程完成 |
