...
Last updated: 2026-05-21 12:10 Asia/Shanghai

## 当前阶段

- 项目处于 V0.4 Edge/Server Runtime 完成阶段。
- 当前仓库已有 Go MVP 骨架、配置样例、迁移样例、RabbitMQ 核心接口、表列映射、MySQL Apply Worker 和无感安装计划模型。
- 当前尚未实现 ACK 持久化、失败事件重放、Canal CDC 和完整 Wails 前端。
...
- V0.4 已完成默认测试门禁；下一步进入 V0.5 CDC stub / Canal 接入前置。
